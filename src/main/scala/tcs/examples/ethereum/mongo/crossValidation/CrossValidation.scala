...
object CrossValidation {
  def main(args: Array[String]): Unit = {
    val blockchain = BlockchainLib.getEthereumBlockchain(new EthereumSettings(CrossValidationSettings.EthereumURL))
    val mongo = new DatabaseSettings(CrossValidationSettings.DatabaseName)
    val weiIntoEth = BigInt("1000000000000000000")
    val myBlockchain = new Collection(CrossValidationSettings.CollectionName, mongo)
